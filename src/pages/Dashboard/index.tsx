  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      try {
        ...
        const apiTransactions = response.data.transactions.map(transaction => {
          const { value, created_at } = transaction;

          return {
            ...transaction,
            formattedValue: formatValue(value),
            formattedDate: formatValue(created_at),
          };
        });

        const apiBalance: Balance = {
          ...response.data.balance,
          formattedIncome: formatValue(response.data.balance.income),
          formattedOutcome: formatValue(response.data.balance.outcome),
          formattedTotal: formatValue(response.data.balance.total),
        };
        ...
        // setApiError('Error requesting transactions');
